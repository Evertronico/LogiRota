"""
Percuros em árvore - Módulo 3 aplicado ao LogiRota
...
Os percursos são genéricos - funcionam sobre qualquer um No, seja ele um
nó da ABB de pontos (Aula 03) ou um nó da hierarqua de zonas (Aula 02).
...
def em_ordem(no, saida=None):
    ...
    percurso que prova, na prática, que a estrutura mantém os elementos
    ordenados sem que nada precise ser ordenado depois.
    """
    if saida is None:
        saida = []
    if no is not None:
...
def pos_ordem(no, saida=None):
    ...
    todas as subzonas já foram - a ordem é inversa da pré-ordem.
    """
    if saida is None:
        saida = []
    if no is not None:
